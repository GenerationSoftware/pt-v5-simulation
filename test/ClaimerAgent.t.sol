...
contract ClaimerAgentTest is Test {
    ...
    function setUp() public {
        ...
        vm.mockCall(address(prizePool), abi.encodeWithSignature("getTierPrizeCount(uint8)", uint8(0)), abi.encode(1));
        vm.mockCall(address(prizePool), abi.encodeWithSignature("getTierPrizeCount(uint8)", uint8(1)), abi.encode(4));
        vm.mockCall(address(prizePool), abi.encodeWithSelector(prizePool.getLastClosedDrawId.selector), abi.encode(1));
